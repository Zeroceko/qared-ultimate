...
const http = axios.create({ baseURL: BINANCE_FAPI_BASE, timeout: 8500, httpsAgent: agent });
...
async function evaluateIntrabarForSymbol({ symbol, tickSize, tickerMap, premiumMap, minConf }) {
  ...
  const liqAdj = applyLiquidationAdjustments(direction, conf, liq); conf = liqAdj.confidence; if (liqAdj.veto) return null; if (Number.isFinite(minConf) && conf < minConf) return null;
...
async function confirmOnCloseForSymbol({ symbol, tickSize, tickerMap, premiumMap, minConfConfirmed }) {
...
export async function handler(event) {
  ...
  const minConf = Number.isFinite(Number(qs.min_conf)) ? Math.max(0, Math.min(100, Number(qs.min_conf))) : CONFIG.MIN_CONF_SHOW;
  const minConfConfirmed = Number.isFinite(Number(qs.min_conf_confirmed)) ? Math.max(0, Math.min(100, Number(qs.min_conf_confirmed))) : minConf; const watchlist = parseWatchlist();
  ...
  try {
    ...
    if (mode === "close") {
      const tasks = watchlist.map((symbol) => confirmOnCloseForSymbol({ symbol, tickSize: tickSizeMap.get(symbol), tickerMap, premiumMap, minConfConfirmed }).catch((err) => ({ id: `ERR|${symbol}`, symbol, mode: "ERROR", error: String(err?.message || err) })));
    ...
    const tasks = watchlist.map((symbol) => evaluateIntrabarForSymbol({ symbol, tickSize: tickSizeMap.get(symbol), tickerMap, premiumMap, minConf }).catch((err) => ({ id: `ERR|${symbol}`, symbol, mode: "ERROR", error: String(err?.message || err) })));
    const results = await Promise.all(tasks);
